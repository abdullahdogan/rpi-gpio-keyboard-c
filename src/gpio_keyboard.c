...
/* ===================== USER MAPPING ===================== */
static map_t MAPS[] = {
    {5,  KEY_ENTER, {0}, 0, "GPIO5->ENTER", 0, 0, 0, 0, 0, 0},
    {27, KEY_ESC,   {0}, 0, "GPIO27->ESC",  0, 0, 0, 0, 0, 0},
    {25, KEY_UP,    {0}, 0, "GPIO25->UP",   0, 0, 0, 0, 0, 0},
    {6,  KEY_DOWN,  {0}, 0, "GPIO6->DOWN",  0, 0, 0, 0, 0, 0},
    {4,  KEY_LEFT,  {0}, 0, "GPIO3->LEFT",  0, 0, 0, 0, 0, 0},
    {7,  KEY_RIGHT, {0}, 0, "GPIO7->RIGHT", 0, 0, 0, 0, 0, 0},
};
/* ========================================================= */
...
/*
  Debounce tuning:
  - If double-press: increase DEBOUNCE_FALL_MS and/or DEBOUNCE_RISE_MS
  - If missed presses: reduce DEBOUNCE_FALL_MS
*/
...
static void emit_mapping(const map_t *m) {
    for (int i = 0; i < m->nmods; i++) uinput_emit_key(m->mods[i], 1);

    uinput_emit_key(m->keycode, 1);
    uinput_emit_key(m->keycode, 0);

    for (int i = m->nmods - 1; i >= 0; i--) uinput_emit_key(m->mods[i], 0);
...
static int setup_uinput(void) {
    ...
    if (ioctl(ufd, UI_SET_EVBIT, EV_KEY) < 0) { perror("UI_SET_EVBIT"); return -1; }

    for (int i = 0; i < (int)(sizeof(MAPS) / sizeof(MAPS[0])); i++) {
        if (ioctl(ufd, UI_SET_KEYBIT, MAPS[i].keycode) < 0) { perror("UI_SET_KEYBIT main"); return -1; }
        for (int j = 0; j < MAPS[i].nmods; j++) {
            if (ioctl(ufd, UI_SET_KEYBIT, MAPS[i].mods[j]) < 0) { perror("UI_SET_KEYBIT mod"); return -1; }
        }
    }
...
static int setup_gpiod_request(void) {
    ...
    if (!settings || !lcfg || !rcfg) { fprintf(stderr, "gpiod alloc failed\n"); return -1; }
    ...
    for (int i = 0; i < (int)(sizeof(MAPS) / sizeof(MAPS[0])); i++) offsets[i] = MAPS[i].offset;

    if (gpiod_line_config_add_line_settings(
            lcfg, offsets, (size_t)(sizeof(offsets) / sizeof(offsets[0])), settings) < 0) {
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (strcmp(argv[i], "--chip") == 0 && i + 1 < argc) { GPIOCHIP_PATH = argv[i + 1]; i++; }
    }

    signal(SIGINT, cleanup);
    signal(SIGTERM, cleanup);

    if (setup_uinput() < 0) { fprintf(stderr, "uinput setup failed\n"); return 1; }
    if (setup_gpiod_request() < 0) { fprintf(stderr, "gpiod request failed\n"); return 1; }

    int gfd = gpiod_line_request_get_fd(req);
    if (gfd < 0) { fprintf(stderr, "gpiod_line_request_get_fd failed\n"); return 1; }

    struct pollfd pfd = { .fd = gfd, .events = POLLIN };
    struct gpiod_edge_event_buffer *buf = gpiod_edge_event_buffer_new(MAX_EVENTS);
    if (!buf) { fprintf(stderr, "gpiod_edge_event_buffer_new failed\n"); return 1; }
    ...
    while (1) {
        ...
        if (rd < 0) { perror("read_edge_events"); continue; }

        for (int i = 0; i < rd; i++) {
            // libgpiod headers use non-const pointer for getters; cast to avoid warnings
            struct gpiod_edge_event *ev =
                (struct gpiod_edge_event *)gpiod_edge_event_buffer_get_event(buf, i);
            ...
            if (et == GPIOD_EDGE_EVENT_FALLING_EDGE) {
                ...
                if (!MAPS[idx].pressed) {
                    MAPS[idx].pressed = 1;
                    MAPS[idx].pressed_ms = t;

                    if (debug) { fprintf(stdout, "FALL %u -> %s\n", off, MAPS[idx].name); fflush(stdout); }
                ...
                if (MAPS[idx].pressed) {
                    ...
                    if (t - MAPS[idx].pressed_ms < MIN_PRESS_MS) {
                        if (debug) { fprintf(stdout, "RISE ignored (bounce) %u\n", off); fflush(stdout); }
                        continue;
                    }

                    if (t - MAPS[idx].last_rise_ms < DEBOUNCE_RISE_MS) continue;
                    MAPS[idx].last_rise_ms = t;

                    // Start release verification (do NOT set pressed=0 immediately)
                    MAPS[idx].release_pending = 1;
                    MAPS[idx].rise_candidate_ms = t;

                    if (debug) { fprintf(stdout, "RISE candidate %u\n", off); fflush(stdout); }
                } else {
                    if (debug) { fprintf(stdout, "RISE ignored (released) %u\n", off); fflush(stdout); }
                }
            }
        }
    }
